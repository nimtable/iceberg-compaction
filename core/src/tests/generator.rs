/*
 * Copyright 2025 BergLoom
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

use crate::{
    CompactionError,
    error::Result,
    executor::{
        datafusion::build_parquet_writer,
        iceberg_writer::{
            self,
            rolling_iceberg_writer::{self, RollingIcebergWriter},
        },
    },
};
use core::pin::Pin;
use futures_async_stream::try_stream;
use iceberg::{
    arrow::schema_to_arrow_schema,
    io::FileIO,
    spec::{DataFile, DataFileBuilder, Schema},
    writer::{
        base_writer::{
            data_file_writer::{self, DataFileWriterBuilder},
            equality_delete_writer::{
                self, EqualityDeleteFileWriterBuilder, EqualityDeleteWriterConfig,
            },
            sort_position_delete_writer::SortPositionDeleteWriterBuilder,
        }, function_writer::equality_delta_writer::EqualityDeltaWriterBuilder, IcebergWriter, IcebergWriterBuilder
    },
};
use parquet::schema;
use rand::{
    Rng,
    distr::{Alphabetic, Alphanumeric},
    rng,
    rngs::StdRng,
    thread_rng,
};
use std::sync::Arc;

use datafusion::arrow::{
    array::{
        Array, ArrayRef, BooleanArray, Float32Array, Float64Array, Int8Array, Int16Array,
        Int32Array, Int64Array, RecordBatch, StringArray, UInt8Array, UInt16Array, UInt32Array,
        UInt64Array,
    },
    compute::filter,
    datatypes::{DataType as ArrowDataType, Field as ArrowField, Schema as ArrowSchema},
};
use futures::{Stream, StreamExt};

struct FieldOption {
    pub field: ArrowField,
    pub length: Option<usize>,
}

pub struct RecordBatchGenerator {
    pub num_rows: usize,
    pub batch_size: usize,
    pub schema: ArrowSchema,
}

impl RecordBatchGenerator {
    pub fn new(num_rows: usize, batch_size: usize, schema: ArrowSchema) -> Self {
        Self {
            num_rows,
            batch_size,
            schema,
        }
    }

    #[try_stream(boxed, ok = RecordBatch, error = CompactionError)]
    pub async fn generate(&self) {
        let mut num_rows = self.num_rows;
        loop {
            if num_rows > self.batch_size {
                let batch = self.generate_batch(self.batch_size as usize)?;
                num_rows -= self.batch_size;
                yield batch;
            } else if num_rows > 0 {
                let batch = self.generate_batch(num_rows as usize)?;
                num_rows = 0;
                yield batch;
            } else {
                break;
            }
        }
    }

    pub fn generate_batch(&self, batch_size: usize) -> Result<RecordBatch> {
        let arrays = self
            .schema
            .fields()
            .iter()
            .map(|field| match field.data_type() {
                datafusion::arrow::datatypes::DataType::Boolean => {
                    Arc::new(BooleanArray::from(vec![true; batch_size])) as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::Int8 => {
                    Arc::new(Int8Array::from(vec![rand::random::<i8>(); batch_size])) as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::Int16 => {
                    Arc::new(Int16Array::from(vec![rand::random::<i16>(); batch_size])) as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::Int32 => {
                    Arc::new(Int32Array::from(vec![rand::random::<i32>(); batch_size])) as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::Int64 => {
                    Arc::new(Int64Array::from(vec![rand::random::<i64>(); batch_size])) as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::UInt8 => {
                    Arc::new(UInt8Array::from(vec![rand::random::<u8>(); batch_size])) as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::UInt16 => {
                    Arc::new(UInt16Array::from(vec![rand::random::<u16>(); batch_size])) as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::UInt32 => {
                    Arc::new(UInt32Array::from(vec![rand::random::<u32>(); batch_size])) as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::UInt64 => {
                    Arc::new(UInt64Array::from(vec![rand::random::<u64>(); batch_size])) as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::Float32 => {
                    Arc::new(Float32Array::from(vec![rand::random::<f32>(); batch_size]))
                        as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::Float64 => {
                    Arc::new(Float64Array::from(vec![rand::random::<f64>(); batch_size]))
                        as ArrayRef
                }
                datafusion::arrow::datatypes::DataType::Utf8 => {
                    Arc::new(StringArray::from(vec![generate_string(16); batch_size])) as ArrayRef
                }
                _ => unimplemented!("Unsupported data type: {:?}", field.data_type()),
            })
            .collect::<Vec<ArrayRef>>();
        RecordBatch::try_new(Arc::new(self.schema.clone()), arrays)
            .map_err(|e| CompactionError::Test(e.to_string()))
    }
}

pub fn generate_string(len: usize) -> String {
    let len = rand::rng().random_range(0..=len);
    rand::rng()
        .sample_iter(&Alphanumeric)
        .take(len)
        .map(char::from)
        .collect()
}
pub struct FileGeneratorBuilder {
    pub data_file_row_count: Option<usize>,
    pub equality_delete_row_count: Option<usize>,
    pub position_delete_row_count: Option<usize>,
    pub data_file_num: Option<usize>,
    pub schema: Option<Arc<Schema>>,
    pub batch_size: Option<usize>,
    pub writer_config: Option<WriterConfig>,
}

impl FileGeneratorBuilder {
    pub fn new() -> Self {
        Self {
            data_file_row_count: None,
            equality_delete_row_count: None,
            position_delete_row_count: None,
            data_file_num: None,
            schema: None,
            batch_size: None,
            writer_config: None,
        }
    }

    pub fn data_file_row_count(mut self, data_file_row_count: usize) -> Self {
        self.data_file_row_count = Some(data_file_row_count);
        self
    }

    pub fn equality_delete_row_count(mut self, equality_delete_row_count: usize) -> Self {
        self.equality_delete_row_count = Some(equality_delete_row_count);
        self
    }

    pub fn data_file_num(mut self, data_file_num: usize) -> Self {
        self.data_file_num = Some(data_file_num);
        self
    }

    pub fn schema(mut self, schema: Arc<Schema>) -> Self {
        self.schema = Some(schema);
        self
    }

    pub fn batch_size(mut self, batch_size: usize) -> Self {
        self.batch_size = Some(batch_size);
        self
    }

    pub fn writer_config(mut self, writer_config: WriterConfig) -> Self {
        self.writer_config = Some(writer_config);
        self
    }

    pub fn build(self) -> Result<FileGenerator> {
        let data_file_row_count = self.data_file_row_count.unwrap_or(10000);
        let equality_delete_row_count = self.equality_delete_row_count.unwrap_or(200);
        let position_delete_row_count = self.position_delete_row_count.unwrap_or(300);
        let data_file_num = self.data_file_num.unwrap_or(10);
        let schema = self.schema.unwrap();
        let batch_size = self.batch_size.unwrap_or(512);
        let writer_config = self.writer_config.unwrap();
        FileGenerator::new(
            data_file_row_count,
            equality_delete_row_count,
            position_delete_row_count,
            batch_size,
            data_file_num,
            writer_config,
            schema,
        )
    }
}
pub struct FileGenerator {
    pub record_batch_generator: RecordBatchGenerator,
    pub data_file_row_count: usize,
    pub equality_delete_row_count: usize,
    pub position_delete_row_count: usize,
    pub data_file_num: usize,
    pub schema: Arc<Schema>,
    pub writer_config: WriterConfig,
}

#[derive(Clone)]
pub struct WriterConfig {
    pub data_file_prefix: String,
    pub dir_path: String,
    pub file_io: FileIO,
    pub equality_ids: Vec<i32>,
}

impl FileGenerator {
    pub fn new(
        data_file_row_count: usize,
        equality_delete_row_count: usize,
        position_delete_row_count: usize,
        batch_size: usize,
        data_file_num: usize,
        writer_config: WriterConfig,
        schema: Arc<Schema>,
    ) -> Result<Self> {
        let arrow_schema = schema_to_arrow_schema(&schema)?;
        let record_batch_generator = RecordBatchGenerator::new(
            data_file_row_count * data_file_num,
            batch_size,
            arrow_schema,
        );
        Ok(Self {
            record_batch_generator,
            data_file_row_count,
            equality_delete_row_count,
            position_delete_row_count,
            data_file_num,
            schema,
            writer_config,
        })
    }

    pub async fn generate(&mut self) -> Result<Vec<DataFile>> {
        let mut stream = self.record_batch_generator.generate().boxed();
        let WriterConfig {
            data_file_prefix,
            dir_path,
            file_io,
            equality_ids,
        } = self.writer_config.clone();
        let parquet_writer_builder = build_parquet_writer(
            data_file_prefix,
            dir_path,
            self.schema.clone(),
            file_io.clone(),
        )
        .await?;
        let data_file_writer_builder =
            DataFileWriterBuilder::new(parquet_writer_builder.clone(), None, 0);
        let position_delete_file_writer_builder = SortPositionDeleteWriterBuilder::new(
            parquet_writer_builder.clone(),
            self.position_delete_row_count as usize,
            None,
            None,
        );
        let equality_delete_file_writer_builder = EqualityDeleteFileWriterBuilder::new(
            parquet_writer_builder.clone(),
            EqualityDeleteWriterConfig::new(equality_ids.clone(), self.schema.clone(), None, 0)?,
        );

        let iceberg_writer_builder = EqualityDeltaWriterBuilder::new(
            data_file_writer_builder,
            position_delete_file_writer_builder,
            equality_delete_file_writer_builder,
            equality_ids,
        );

        let mut data_files = Vec::new();

        let mut iceberg_writer = iceberg_writer_builder.clone().build().await?;

        let equality_delete_rate = self.equality_delete_row_count / self.data_file_row_count;
        let position_delete_rate = self.position_delete_row_count / self.data_file_row_count;

        let mut data_file_num = 0;

        let mut cache_equality_delete_batch = Vec::new();
        let schema_with_extra_op_column = {
            let arrow_schema = schema_to_arrow_schema(&self.schema)?;
            let mut new_fields = arrow_schema.fields().iter().cloned().collect::<Vec<_>>();
            new_fields.push(Arc::new(ArrowField::new(
                "op".to_owned(),
                ArrowDataType::Int32,
                false,
            )));
            Arc::new(ArrowSchema::new(new_fields))
        };

        for batch in stream.next().await {
            let batch = batch?;
            let num_rows = batch.num_rows();

            if data_file_num + num_rows > self.data_file_row_count {
                let cache_equality_delete_batch =
                    cache_equality_delete_batch.drain(..).collect::<Vec<_>>();
                for batch in cache_equality_delete_batch {
                    iceberg_writer.write(batch).await?;
                }
                data_files.extend(iceberg_writer.close().await?);
                iceberg_writer = iceberg_writer_builder.clone().build().await?;
                data_file_num = 0;
            }

            // build equality delete mask
            let mask =
                BooleanArray::from_iter((0..num_rows).map(|i| Some(i % equality_delete_rate == 0)));

            let mut filtered_columns: Vec<ArrayRef> = batch
                .columns()
                .iter()
                .map(|col| filter(col, &mask).unwrap())
                .collect();
            filtered_columns.push(Arc::new(Int32Array::from(vec![2; num_rows])) as ArrayRef);

            let equality_delete_batch =
                RecordBatch::try_new(schema_with_extra_op_column.clone(), filtered_columns)
                    .map_err(|e| CompactionError::Test(e.to_string()))?;
            cache_equality_delete_batch.push(equality_delete_batch);

            // build position delete mask
            let mask =
                BooleanArray::from_iter((0..num_rows).map(|i| Some(i % position_delete_rate == 0)));

            let mut filtered_columns: Vec<ArrayRef> = batch
                .columns()
                .iter()
                .map(|col| filter(col, &mask).unwrap())
                .collect();
            filtered_columns.push(Arc::new(Int32Array::from(vec![2; num_rows])) as ArrayRef);

            let position_delete_batch =
                RecordBatch::try_new(schema_with_extra_op_column.clone(), filtered_columns)
                    .map_err(|e| CompactionError::Test(e.to_string()))?;
            iceberg_writer.write(position_delete_batch).await?;

            let mut columns = batch.columns().to_vec();
            columns.push(Arc::new(Int32Array::from(vec![1; num_rows])) as ArrayRef);
            let batch = RecordBatch::try_new(schema_with_extra_op_column.clone(), columns)
                .map_err(|e| CompactionError::Test(e.to_string()))?;
            iceberg_writer.write(batch).await?;
        }
        for batch in cache_equality_delete_batch {
            iceberg_writer.write(batch).await?;
        }
        data_files.extend(iceberg_writer.close().await?);
        Ok(data_files)
    }
}
